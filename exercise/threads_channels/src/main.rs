...
fn challenge_print<'a>(msg: &'a str, tx: Sender<&'a str>) {
    tx.send(msg).unwrap();
    sleep_ms(500);
}
...
fn main() {
    ...
    // 1. Spawn a child thread and have it call `expensive_sum(my_vector)`.
    // Store the returned join handle in a variable called `handle`.
    // Once you've done this you should be able to run the code and
    // see the output from the child thread's expensive sum in the middle of the main
    // thread's processing of letters.
    let handle = thread::spawn(move || expensive_sum(my_vector));

    // While the child thread is running the main thread will also do somw work
    ...
    //   unbounded channel.
    //
    // Hint: An unbounded channel can be created with `channel::unbounded()`
    ...
    // Main thread 가 자는 동안, Thread A는 깨어있습니다.
    ...
    let challenge_handle = thread::spawn(move || challenge_print("Challenge Thread", c_tx2));
